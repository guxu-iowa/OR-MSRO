...
function [sdpval, sdpx, run_time] = newsvendor_qdr(A, B, d, F, P)
...
con = [ con; ll*(e1)*(e1)' - T1 == S + P'*N*P; S >= 0; N(:) >= 0 ];
...
for i = 1 : m
    ...
    con = [ con; (A(i,:)*x-pi(i))*(e1)*(e1)' + Ti - 0.5*F(i,:)'*(e1)' - 0.5*(e1)*F(i,:) == SS(:,:,i) + P'*NN(:,:,i)*P; SS(:,:,i) >= 0 ];
    for j = 1 : s
        con = [ con; NN(j,:,i) >= 0 ];
    end
end
