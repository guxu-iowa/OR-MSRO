function [obj_val, run_time, ret_code] =  approxSL(data)
...
cons = [ cons ; ll*(eKp1)*eKp1' - Mtx == 0.5*(P'*Tht*eKp1' + eKp1*Tht'*P) + V ];
cons = [ cons ; V >= 0 ; Tht >= 0 ];
...
for t = 1 : T
    ...
    cons = [ cons ; PJt'*W{1,t}*PJt + PJt'*S{1,t}*PJt - 0.5*(eKp1*FF' + FF*eKp1') - piWT(t)*(eKp1)*eKp1' == 0.5*(P'*ThWT(:,t)*eKp1' + eKp1*ThWT(:,t)'*P) + VWT(:,:,t); VWT(:,:,t) >= 0; piWT(t) >= 0; ThWT(:,t) >= 0 ];
    
    cons = [ cons ; PJt'*W{1,t}*PJt - PJt'*S{1,t}*PJt + 0.5*(eKp1*FF' + FF*eKp1') - piWTm(t)*(eKp1)*eKp1' == 0.5*(P'*ThWTm(:,t)*eKp1' + eKp1*ThWTm(:,t)'*P) + VWTm(:,:,t) ; VWTm(:,:,t) >= 0; piWTm(t) >= 0; ThWTm(:,t) >= 0 ];
end
...
for t = 1 : T
    ...
    cons = [ cons ; PJt'*S{1,t}*PJt - Mtx - piSUT(t)*(eKp1)*eKp1' == 0.5*(P'*ThSUT(:,t)*eKp1' + eKp1*ThSUT(:,t)'*P) + VSUT(:,:,t) ; VSUT(:,:,t) >= 0; piSUT(t) >= 0; ThSUT(:,t) >= 0 ];
end
